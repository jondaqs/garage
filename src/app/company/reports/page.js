...
/**
 * Company reports page (owner portal).
 *
 * Symmetric to /dashboard/company/[id]/reports — same cards, same
 * panels, same metrics. This one resolves the company from the
 * current user's company_profiles.owner_user_id (no URL param needed)
 * and skips the role-tier check since middleware already restricts
 * /company/* to the company owner.
 *
 * Operational overview — fleet-wide booking + work-order activity.
 * Financial views (spend, budget) live on /company/budget, so this
 * page focuses on workflow state, provider relationships, downtime,
 * and average service metrics.
 *
 * Layout:
