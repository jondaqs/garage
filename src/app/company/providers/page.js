...
export default function ProvidersPage() {
  ...
  }, [ownerAccess.companyId, search, descSearch, typeFilter, locationFilter, countryFilter, verifiedOnly, page])
  ...
  const handleChat = async (e, provider) => {
    e.stopPropagation()
    router.push(`/company/chat?provider=${provider.id}`)
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">{total} provider{total !== 1 ? 's' : ''} available</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {providers.map(p => (
                <ProviderCard
                  key={p.id}
                  provider={p}
                  onClick={() => router.push(`/company/providers/${p.id}`)}
