...
const statusColor = (code) =>
  STATUS_COLORS[code] || { bg: '#6b7280', border: '#4b5563' }
...
export default function CompanyCalendarPage() {
  ...
  // ── Resolve company + load bookings + active WOs ─────────────────────────
  const loadData = async () => {
    try {
      ...
      // Active work orders (non-terminal)
  ...
  const events = useMemo(() => {
    const bookingEvents = bookings
      ...
      .map(b => {
        ...
        const end = new Date(b.booking_date)
        end.setHours(parseInt(eh), parseInt(em), 0)
        return {
          id:       b.id,
          title:    `${b.vehicle?.plate_number || '—'} · ${b.provider?.name || '—'}`,
          start,
          end,
          resource: b,
          kind:     'booking',
    ...
    const woEvents = showWOs
      ? workOrders
          ...
          .map(wo => {
            // Show as an all-day event on opened_at date
            const d = new Date(wo.opened_at || wo.updated_at)
            const start = new Date(d.getFullYear(), d.getMonth(), d.getDate())
            const end   = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1)
            return {
              id:       wo.id,
              title:    `🔧 ${wo.vehicle?.plate_number || '—'} · ${wo.status?.display_name}`,
              start,
              end,
              allDay:   true,
              resource: wo,
  ...
  const handleSelectEvent = (event) => {
    if (event.kind === 'work_order') {
      router.push(`/company/work-orders/${event.id}`)
    } else {
      router.push(`/company/bookings/${event.id}`)
    }
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => router.push('/company/bookings/book')}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <p className="text-sm text-blue-800">
          <strong>💡 Tip:</strong> Click any booking to view details, or click an empty date
          to schedule a new fleet service booking. Work order events (all-day banners) link
