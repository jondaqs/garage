// → Drop this file at: src/app/provider/providers/page.js
'use client'
...
export default function ProviderSearchProvidersPage() {
  ...
  const [startingChat, setStartingChat] = useState(null) // provider id while RPC pending
  ...
  // ── Start (or resume) a peer-provider chat via RPC ──
  // The RPC enforces:
  //   - caller must be a chat-able member of some provider
  //   - target must be a different, active provider
  //   - returns the existing peer_conversation if one exists in either direction
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {providers.map(p => (
                <ProviderCard
                  ...
                  onClick={() => router.push(`/provider/providers/${p.id}`)}
