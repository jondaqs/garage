...
import {
  ...
  Loader2, DollarSign, Printer, Send
} from 'lucide-react'
...
export default function InvoiceTab({ workOrder }) {
  ...
  const canGenerate = ['completed','closed','quality_check'].includes(statusCode)

  const loadInvoice = useCallback(async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser()
      const { data: result, error: rpcErr } = await supabase.rpc('get_invoice_details', {
        p_invoice_id:      null,   // need to find by work_order_id first
        p_requesting_user: user.id,
      })
      // Fallback: query directly by work_order_id
      const { data: inv } = await supabase
        .from('invoices')
        .select('id, invoice_number, status, subtotal, tax_rate, tax_amount, total_amount, notes, due_date, issued_at, paid_at')
        .eq('work_order_id', workOrder.id)
        .maybeSingle()

      if (inv) {
        // Load full details via RPC
        ...
        if (details?.success) {
          ...
          setInvoice({ invoice: inv, line_items: null, receipt: null })
        }
      } else {
        setInvoice(null)
      }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setSuccess(`Invoice ${result.invoice_number} generated. Customer has been notified.`)
          ...
          {canGenerate ? (
            <>
              <p className="text-xs text-gray-400 mb-4">
                Invoice will be calculated from actual service costs and parts used.
              </p>
              <button
            ...
            <p className="text-xs text-gray-400">
              Invoice can be generated once the work order is completed.
              Current status: <strong>{workOrder.status?.display_name}</strong>
            </p>
          )}
        </div>
      )}
      ...
      {invoice?.invoice && (
        <div className="space-y-4">
          ...
          <div className="flex items-start justify-between">
            <div>
              <h3 className="font-bold text-gray-900">{invoice.invoice.invoice_number}</h3>
              <p className="text-xs text-gray-500 mt-0.5">
                Issued {new Date(invoice.invoice.issued_at).toLocaleDateString('en-KE', {
                  day: 'numeric', month: 'short', year: 'numeric'
                })}
                {invoice.invoice.due_date && (
                  <> · Due {new Date(invoice.invoice.due_date).toLocaleDateString('en-KE', {
                    day: 'numeric', month: 'short', year: 'numeric'
                  })}</>
                )}
              </p>
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-2">
            <div className="flex justify-between text-sm text-gray-600">
              <span>Subtotal</span><span>{fmt(invoice.invoice.subtotal)}</span>
            </div>
            {invoice.invoice.discount > 0 && (
              <div className="flex justify-between text-sm text-green-700">
                <span>Discount</span><span>−{fmt(invoice.invoice.discount)}</span>
              </div>
            )}
            <div className="flex justify-between text-sm text-gray-600">
              <span>VAT ({Math.round((invoice.invoice.tax_rate || 0.16) * 100)}%)</span>
              <span>{fmt(invoice.invoice.tax_amount)}</span>
            </div>
            <div className="flex justify-between text-base font-bold text-gray-900 pt-2 border-t border-gray-200">
              <span>Total</span>
              <span className="text-green-700">{fmt(invoice.invoice.total_amount)}</span>
            </div>
          </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                <div>
                  ...
                  <p className="font-medium text-gray-900">{invoice.receipt.receipt_number}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Method</p>
                  <p className="font-medium text-gray-900 capitalize">{invoice.receipt.payment_method}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Amount Paid</p>
                  <p className="font-medium text-gray-900">{fmt(invoice.receipt.amount_paid)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Date</p>
                  <p className="font-medium text-gray-900">
                    {new Date(invoice.receipt.paid_at).toLocaleDateString('en-KE', {
                      day: 'numeric', month: 'short', year: 'numeric'
                    })}
                  </p>
                </div>
            ...
            <div>
              {!showPayForm ? (
                <button
                  onClick={() => {
                    setShowPayForm(true)
                    setAmountPaid(invoice.invoice.total_amount?.toString() || '')
                  }}
                ...
                <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 space-y-3">
                  ...
                  {/* Change calculation */}
                  {parseFloat(amountPaid) > invoice.invoice.total_amount && (
                    <div className="text-xs text-green-700 bg-green-50 px-3 py-2 rounded-lg">
                      Change to give: {fmt(parseFloat(amountPaid) - invoice.invoice.total_amount)}
                    </div>
                  )}
