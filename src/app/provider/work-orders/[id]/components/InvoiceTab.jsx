...
export default function InvoiceTab({ workOrder, permissions = null }) {
  ...
  const [vatPct,       setVatPct]       = useState(String(workOrder?.vat_rate ?? 0))
  ...
  // PDF download — uses the shared hook so this surface stays in lock-step
  // with the customer-side invoice pages and the email attachment.
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">From work order: {workOrder?.vat_rate ?? 0}%</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">
                    Discount (%)
    ...
    <div className="space-y-4">
      ...
      {(error || dlError) && (
        <div className="p-3 bg-red-50 border border-red-200 rounded-xl flex items-start gap-2 text-sm">
          ...
          <span className="text-red-700">{error || dlError}</span>
        </div>
      )}
      {success && (
      ...
      <div className="flex justify-end">
        <button
          onClick={() => downloadPdf({ workOrderId: workOrder.id, invoiceNumber: inv.invoice_number })}
        ...
        <div className="border-t border-gray-100 px-6 py-4 bg-gray-50">
          <div className="space-y-2 max-w-xs ml-auto">
            ...
            <div className="flex justify-between text-sm text-gray-500">
              <span>VAT ({Math.round((inv.tax_rate || (workOrder?.vat_rate ?? 0) / 100) * 100)}%)</span>
