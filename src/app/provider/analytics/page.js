...
export default function ProviderAnalyticsPage() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      // PostgREST foreign-table filter: rows with non-matching work_order come back with work_order: null
      ...
      invoices.forEach(inv => {
        ...
        if (day) {
          const key = day
          if (!dailyRevenue[key]) dailyRevenue[key] = {}
          dailyRevenue[key][cc] = (dailyRevenue[key][cc] || 0) + amt
        }
      })
  ...
  // Revenue for the active currency filter
  ...
  const DailyRevenueChart = () => {
    ...
    // dailyRevenue is { day: { KES: 1000, USD: 50 }, ... }
    // Flatten to entries filtered by active currency
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Currency filter (only when multiple currencies) */}
      ...
      <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2 text-sm">
        ...
        <p className="text-amber-800">
          ...
          For paid receipt totals, visit the <a href="/provider/revenue" className="underline font-medium hover:text-amber-900">Revenue page</a>.
        </p>
      </div>
          ...
          {Object.keys(data.dailyRevenue || {}).length > 1 && (
            <div className="bg-white rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-1">
                ...
                <p className="text-base font-bold text-green-700">{fmt(displayRevenue())}</p>
              </div>
              ...
              <div className="space-y-3">
                {data.shopPerformance.map((s, i) => (
                  <Bar key={i}
                    ...
                    label={s.name}
                    amount={fmt(s.revenue, s.currency)}
