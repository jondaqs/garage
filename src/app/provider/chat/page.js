  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      ...
      .on('postgres_changes', {
        ...
        // immediately after inserting the message. If the provider has the
        // conversation open we don't want that bump to surface — they're
        // looking at it. So we reset both the message's is_read flag AND the
        // conversation's counter. Both updates are fire-and-forget; we also
        // optimistically zero the counter in local state so the badge doesn't
        // flicker between the customer's bump arriving and our zero landing.
      ...
      .on('postgres_changes', {
        ...
        // badge for a conversation the provider is actively reading. Our
        // INSERT handler above is what does the authoritative DB reset; this
        // local override just keeps the rendered state truthful in between.
          ...
          <>
            ...
            <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 flex-shrink-0">
              ...
              <ChatAvatar
                ...
                name={`${activeConv.user?.first_name || ''} ${activeConv.user?.last_name || ''}`.trim() || 'Customer'}
                size="sm"
                gradient="from-gray-400 to-gray-600"
              />
              <div className="flex-1 min-w-0">
