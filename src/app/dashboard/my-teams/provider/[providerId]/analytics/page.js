...
export default function MemberAnalyticsPage() {
  ...
  useEffect(() => { checkAccessAndLoadShops() }, [])
  useEffect(() => { if (!accessDenied) loadAnalytics() }, [days, shopId, accessDenied])

  const checkAccessAndLoadShops = async () => {
    try {
      ...
      if (!spuRow) { setAccessDenied(true); setLoading(false); return }
        ...
        if (ownerships && ownerships.length > 0) {
          ...
          if (topCompanyIds.length > 0) {
            const { data: companyProfiles } = await supabase
              .from('company_profiles').select('id, name').in('id', topCompanyIds.map(c => c.companyId))
            const compMap = {}
            ;(companyProfiles || []).forEach(c => { compMap[c.id] = c.name })
            topCompanies = topCompanyIds.map(c => ({ ...c, name: compMap[c.companyId] || 'Unknown Company' }))
          }
        }
      }
  ...
  const DailyRevenueChart = () => {
    ...
    return (
      <div ref={dailyScrollRef} className="overflow-x-auto pb-2 -mx-2 px-2" style={{ scrollbarWidth: 'thin' }}>
              ...
              <div key={day} className="flex flex-col items-center flex-1" style={{ minWidth: '40px' }}>
                ...
                <div className="w-full flex justify-center">
                  <div className="w-7 bg-blue-400 rounded-t hover:bg-blue-500 transition-colors cursor-default"
                    style={{ height: `${barH}px` }} title={`${day}: ${fmt(val)}`} />
                </div>
                <span className="text-[9px] text-gray-400 mt-1.5 whitespace-nowrap">{formatDay(day)}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-1">
              Analytics is only available for admin, accountant, and owner roles.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <TrendingUp size={24} className="text-blue-600" /> Analytics
          </h1>
          <p className="text-sm text-gray-500 mt-1">Performance overview for {providerName}</p>
        </div>
              ...
              {data.topServices.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.topServices.map((svc, i) => (
                    <Bar key={i} value={svc.count} max={data.topServices[0].count} label={svc.name}
                      amount={svc.revenue > 0 ? fmt(svc.revenue) : null}
                      color={['bg-blue-500','bg-indigo-500','bg-purple-500','bg-orange-500',
                ...
                <div className="space-y-3">
                  {[5,4,3,2,1].map(n => (
                    <div key={n} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-gray-600 w-6 text-right flex-shrink-0">{data.ratingDist[n]}</span>
                    </div>
                  ))}
                    ...
                    {data.mechanicPerformance.map((m, i) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="text-center py-3 px-2">
                          ...
                          }`}><Car size={12} />{m.assignedVehicles}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
