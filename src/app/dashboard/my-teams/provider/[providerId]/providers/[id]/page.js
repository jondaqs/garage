// → Drop this file at: src/app/dashboard/my-teams/provider/[providerId]/providers/[id]/page.js
'use client'

/* ============================================================================
 * Provider-member provider detail — scoped to ONE membership via the route.
 *
 * Path: /dashboard/my-teams/provider/[providerId]/providers/[id]
 *
 *   • providerId = the provider whose hat the caller is currently wearing
 *   • id         = the provider being viewed
 *
 * Same shape as the owner-side /provider/providers/[id] but the chat CTA
 * passes the explicit p_own_provider_id and routes back into the member-
 * scoped peer-chat page.
 * ============================================================================ */
...
export default function MemberProviderPeerDetailPage() {
  const router  = useRouter()
  const params  = useParams()
  const supabase = createClient()
  const ownProviderId    = params?.providerId
  const targetProviderId = params?.id
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!p) {
        router.replace(`/dashboard/my-teams/provider/${ownProviderId}/providers`)
        return
      }

      // Sanity check: catch direct-URL navigation to a provider the caller belongs to.
  ...
  }, [targetProviderId, ownProviderId])
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {provider.is_verified && <BadgeCheck size={22} className="text-green-400 flex-shrink-0" />}
              </div>
              <p className="text-green-300 text-sm mt-1 font-medium">{provider.provider_type?.display_name}</p>
          ...
          <div className="flex gap-3 mt-6">
            {isOwnProvider ? (
              <div className="px-5 py-2.5 bg-white/10 rounded-xl text-sm text-white/70 border border-white/20">
                You're a member of this provider — can't peer-chat with yourself.
              </div>
            ) : (
              <button onClick={handleChat} disabled={startingChat}
            ...
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${
                  tab === t.id ? 'border-green-400 text-white' : 'border-transparent text-white/50 hover:text-white/80'
                }`}>
                {t.label}
              </button>
            ))}
          ...
          <div className="space-y-6">
            ...
            {provider.shops?.[0]?.latitude && provider.shops?.[0]?.longitude && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <iframe title="Provider location" width="100%" height="240"
              ...
              provider.shops.map((shop, i) => (
                <div key={shop.id || i} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  {shop.latitude && shop.longitude ? (
                    <iframe title={shop.name || `Shop ${i + 1}`} width="100%" height="220"
                      style={{ border: 0, display: 'block' }} loading="lazy"
                      referrerPolicy="no-referrer-when-downgrade"
