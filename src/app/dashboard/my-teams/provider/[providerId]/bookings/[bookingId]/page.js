...
const ADMIN_ROLES = new Set(['service_provider_owner', 'admin', 'accountant'])

/**
 * Member-side booking detail page.
 *
 * Permission rules — implemented via 4 derived flags:
 *
 *   canAcceptBooking ─ admin/accountant role OR can_approve_work
 ...
 *
 *   canManageWO      ─ admin/accountant role OR member is the assigned mechanic
...
export default function MemberBookingDetailPage() {
  ...
  const isAdminRole         = membership && ADMIN_ROLES.has(membership.role)
  ...
  const canManageWO         = !!membership && (isAdminRole || isAssignedMechanic)
  ...
  const acceptAndCreateWorkOrder = async () => {
    ...
    try {
      ...
      const willBeAbleToManage =
        isAdminRole || (myMechanicId && newWoAssignedTo === myMechanicId)
