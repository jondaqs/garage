// → Drop this file at: src/app/dashboard/company/[companyId]/providers/[id]/page.js
'use client'

/* ============================================================================
 * Provider Detail — team-company member surface.
 *
 * Path: /dashboard/company/[companyId]/providers/[id]
 *
 * Same UI as /company/providers/[id] (the company-owner equivalent), but the
 * Chat / Book CTAs route into the team-company surface so actions taken here
 * are scoped to the company, not to the member's personal account.
 * ============================================================================ */
...
export default function ProviderDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!p) { router.replace(`/dashboard/company/${companyId}/providers`); return }
  ...
  const handleChat = async () => {
    ...
    router.push(`/dashboard/company/${companyId}/chat?provider=${id}`)
  }

  const handleBook = () => {
    router.push(`/dashboard/company/${companyId}/bookings/book?provider=${id}`)
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-blue-900 text-white">
        <div className="max-w-4xl mx-auto px-4 pt-6 pb-8">
          <button onClick={() => router.push(`/dashboard/company/${companyId}/providers`)}
          ...
          <div className="flex items-start gap-5">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-400 to-blue-700 flex items-center justify-center text-white font-bold text-2xl shadow-xl flex-shrink-0">
          ...
          <div className="flex gap-3 mt-6">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-semibold transition-colors border border-white/20"
            >
            ...
            <button
              onClick={handleBook}
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-400 rounded-xl text-sm font-semibold transition-colors shadow-lg"
            >
              <Calendar size={15} />
              Book Service
            </button>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              {provider.reviewCount > 0 && (
                <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-center">
                  <p className="text-3xl font-bold text-blue-500">{provider.reviewCount}</p>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {services.slice(0, 10).map(s => (
                    <span key={s.id}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-xl text-xs font-medium">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            {services.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {services.map(s => (
                  <div key={s.id} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl hover:bg-blue-50 transition-colors">
                  ...
                  {paginatedReviews.map(r => {
                    ...
                    return (
                      <div key={r.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                        ...
                        {r.provider_response && (
                          <div className="mt-3 pl-4 border-l-2 border-blue-200 bg-blue-50 rounded-r-xl p-3">
