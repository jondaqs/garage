...
/**
 * Company reports — dashboard portal view.
 *
 * Mirrors /company/reports (operational cards + Bookings/WO/Providers/Team)
 * and adds two deeper analysis panels for admins, accountants, and
 * payment-approvers:
 *
 *   • Vehicle Utilization / Downtime — per-vehicle days with a
 *     provider in the last 30 + 90 days, derived from work_orders'
 *     vehicle_checked_in_at / vehicle_checked_out_at columns (or
 *     opened_at / closed_at when those aren't recorded).
 *
 *   • Average Service Metrics — per-currency average service cost +
 *     fleet-wide average duration (check-in → closed). Grouped by
 *     currency because mixing them into one number is meaningless;
 *     duration is currency-agnostic so it's one figure.
 *
 * Access tiers match the budget surface:
 *   admin / accountant / can_approve_payment → see the page
 *   anyone else → lock screen.
 */
...
export default function CompanyReportsDashboardPage() {
  ...
  const [utilization,    setUtilization]    = useState([])  // [{vehicle, days30, days90}]
  const [avgMetrics,     setAvgMetrics]     = useState(null)
  // avgMetrics shape:
  //   {
  //     duration_days_avg: number | null,
  //     duration_sample:   number,
  //     by_currency: [{currency, avg, count, total}]
  //   }
  ...
  const fetchData = async () => {
    try {
      ...
      // page-level check matches so direct URLs are also gated. If you
      // later want to widen this (e.g. to accountants + payment-approvers,
      // matching Budget), it's a one-line change here AND the matching
      // sidebar filter.
      ...
      const access = !!mem.is_admin
      setCanView(access); setAccessChecked(true)
      ...
      // Team roster (regardless of fleet size).
      ...
      // For each WO, the vehicle was "with a provider" between
      // vehicle_checked_in_at and vehicle_checked_out_at (fall back to
      // opened_at/closed_at if check-in/out wasn't recorded). We sum
      // the overlap with each window per vehicle.
      const now        = new Date()
      ...
      // Sort: most downtime first (a fleet manager's "look at me" list).
      ...
      // 4. Average service metrics
      // Duration: closed_at - check-in (fall back to opened_at).
      // Only count terminal WOs that actually finished.
      // Cost: per-currency avg of total_amount across the same set.
      let durSum = 0, durN = 0
  ...
  if (accessChecked && !canView) return (
    <div className="max-w-md mx-auto mt-16 text-center">
      ...
      <p className="text-gray-500 text-sm">
        Only company admins can view reports.
      </p>
    </div>
  )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Operational overview of fleet activity. For spend tracking see Budget.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Vehicle utilization / downtime — NEW */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:col-span-2">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Lower is better — these are days a vehicle was off the road
            for service in the last 30 / 90 days.
          </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {utilization.map(({ vehicle, days30, days90, visits90 }) => {
                    ...
                    // 7+ days in 30d is meaningful downtime (>23%);
                    // colour it amber to flag for the operator.
          ...
          {!avgMetrics || (avgMetrics.by_currency.length === 0 && avgMetrics.duration_sample === 0) ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {/* If there's only one currency, surface its avg here for
                    parity with the duration card. Multi-currency case
