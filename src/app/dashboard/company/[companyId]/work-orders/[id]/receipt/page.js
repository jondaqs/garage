...
function ReceiptPageInner({ backPath }) {
  ...
  const backTo = backPath.replace('[companyId]', params.companyId).replace('[id]', params.id)
...
// ── Company dashboard receipt page ───────────────────────────────────────────────
export default function CompanyReceiptPage() {
  return <ReceiptPageInner backPath="/dashboard/company/[companyId]/work-orders/[id]" />
}