  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white border-r border-gray-200 overflow-y-auto">
      ...
      <div className="flex-shrink-0 border-t border-gray-200 p-4">
        <button
          onClick={() => { router.push('/provider/feedback'); setMobileOpen(false) }}
        ...
        <button
          onClick={() => { router.push('/provider/service-marketplace'); setMobileOpen(false) }}
          className="w-full group flex items-center px-3 py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-lg transition-colors mb-1">
          <Megaphone className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" />
          Service Marketplace
        </button>
