    ...
    try {
      const { data: authData, error: signupError } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: `${window.location.origin}/auth/callback?next=/auth/company-signup`,
          data: {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/auth/company-signup`,
          queryParams: {
            user_type: 'company'
          }
        }
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <p className="text-gray-600 mb-6">
        {isLogin 
          ? 'Sign in to continue with company registration' 
      ...
      <form onSubmit={isLogin ? handleLogin : handleSignup}>
        ...
        />
        <Script id="turnstile-callbacks-company-auth" strategy="afterInteractive">
          {`
